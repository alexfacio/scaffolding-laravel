...
ajaxSettings.url=url;var target=(settings.target)?settings.target:container;var new_elem=(container.is('table'))?jQuery('<tbody/>'):jQuery('<span/>');if(settings.newElementClass!=''){jQuery(new_elem).addClass(settings.newElementClass);}
if(settings.loadingIndicatorEnabled){var jqLoadingElem=jQuery('<spab></span>');jqLoadingElem.addClass(settings.loadingIndicatorClass);jqLoadingElem.appendTo(target);}
