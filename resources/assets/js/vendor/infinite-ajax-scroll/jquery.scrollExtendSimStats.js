	...
	jQuery(this).onScrollBeyond(
		function(container) {
			...
			if ( jqContainerElem.data('scrollExtend-disabled') != true && 
				...
				//by cuauh var new_elem = ( container.is('table') ) ? jQuery('<tbody/>') : jQuery('<div/>');
				//Se cambio esta linea por que daba problemas con el diseño de perfiles.clubgaleria.com
				var new_elem = ( container.is('table') ) ? jQuery('<tbody/>') : jQuery('<span/>');
				var new_elem2 = ( container.is('table') ) ? jQuery('<tbody/>') : jQuery('<span/>');
				...
				localAjaxSettings = {
					'success': function(data, textStatus) {
						//console.log(data.viewStat);
						
						var target = ( settings.target ) ? settings.target : container;
					
						jQuery(new_elem).html(data.view);
						jQuery(new_elem).appendTo(target);
						
						jQuery(new_elem2).html(data.viewStat);
						jQuery(new_elem2).appendTo(jQuery('#ajax_grafica'));
						...
						$.each(data.productos, function(index, value){
								var percent = 0;
								if( parseInt(value.count_alcance) > 0 ){
									percent = (parseInt(value.count_ventas)/parseInt(value.count_alcance))*100;
								}
								//alert('id:'+value.id+' ventas:'+value.count_ventas+' alcance:'+value.count_alcance+' percent:'+percent);
								createChart(parseInt(value.id), parseInt(value.count_ventas), parseInt(value.count_alcance), percent)
							});
